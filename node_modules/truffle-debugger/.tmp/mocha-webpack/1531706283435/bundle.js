...
function toBytes(number, length = 0) {
  ...
  if (number < 0 || number === "") {
    return [];
  } else if (typeof number === "string" && number.slice(0, 2) != "0x") {
    bytes = new Uint8Array(number.split("").map(c => c.charCodeAt(0)));
  } else {
...
function slotAddress(slot) {
  if (slot instanceof Array) {
    ...
    return utils.toBigNumber(utils.toBytes(slot));
  } else {
